...
date2='05162025'
dirout='/glade/derecho/scratch/ito/ML4BGC_temp/'
...
#                                   min_samples_leaf=msl,max_features='sqrt')
    ...
    #
    np.savez(dirfin+f'test_pred_v{ver}.npz',Xtest=X_test,test=y_test*tmp['ystd']+tmp['ym'],est=y_est*tmp['ystd']+tmp['ym'])
    #
...
def rev_trans_validation(basin,nver):
    ...
    test = np.load(f'{dirfin}test_pred_v{ver}.npz')
    R2=np.zeros(5)
    RMSE=np.zeros(5)
    #
...
ax.plot([0,4],[0,4],color='k')
tmp = np.corrcoef(o2test,o2est)
r2 = np.round(tmp[0,1],3)
rmse = np.round(np.sqrt(np.mean((o2test-o2est)**2)),3)
ax.text(0,3,f'R2 ={r2}')
ax.text(0,3,f'RMSE ={rmse}')
ax.set_xlabel('test data, micro-mol/kg')
ax.set_ylabel('estimates by algorithm, micro-mol/kg')
