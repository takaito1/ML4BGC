...
dirout='/glade/derecho/scratch/ito/ML4BGC_results/'
# 
...
def map_yearly(year):
    ...
    da1=xr.DataArray(data=o2est2,name='po4est',dims=['time','depth','lat','lon'],
                 coords={'time':t,'depth':zlev_arr,'lat':yout,'lon':xout})
    ds=da1.to_dataset()
    ds.to_netcdf(diro+f'temp/po4est_v{ver}_{year}.nc')
    return 0
...
for zlev_cnt,zlev in enumerate(zlevels):
    ...
    #kind=[idx for idx,elem in enumerate(Z) if elem==zlev]
    #maz=np.squeeze(ma[kind,:,:])
    os.system('rm '+diro+f'/temp/po4est_v{ver}_*.nc')
    os.system('rm '+diro+f'/PO4map_v{ver}_z{int(zlev)}.nc')
    #
    ...
    #
    dtemp=xr.open_mfdataset(diro+f'temp/po4est_v{ver}_*.nc')
    dtemp.to_netcdf(diro+'/PO4map_v'+ver+'_z'+str(int(zlev))+'.nc')


# In[ ]:


ds=xr.open_mfdataset(f'{diro}PO4map_v{ver}*')


# In[ ]:


ds.to_netcdf(f'{dirout}PO4map_v{ver}.nc')
